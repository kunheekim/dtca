...
output datasets:
	...
	tv_cpm_exp2000.dta - tv_cpm_exp2009.dta
...
*merge TV CPM data with DTC ad spending, # ads (ad_bydrg_dma_coll_wide`yr') data
foreach yr of numlist 2000/2009 {
	use tv_cpm`yr', clear
	drop rank
	order dmacode dma state census_region cpp_cpm
	*in merging, drop DMAs that have missing values for tv (tvu) in any year	
	*merge m:1 dmacode using "/Users/kunheekim/Documents/Kessler/dtca/IVdata/stata_data/ad_bydrg/ad_bydrg_dma_coll_wide`yr'"
	merge m:1 dmacode using "/Users/kunheekim/Documents/kessler/dtca/IVdata/stata_data/ad_exp`yr'", keep(3) nogen keepusing(tv tvu mag magu)	
	replace tv = tv*1000
	replace mag = mag*1000
	*generate average tv (print) ad spending in a DMA-year
	gen avg_tvad_exp = tv/tvu
	gen avg_printad_exp = mag/magu
	mvencode avg_tvad_exp if tvu==0, mv(0) 
	mvencode avg_printad_exp if magu==0, mv(0) 
	order dmacode dma state census_region hh_number cpp_cpm earlymorn-latefringe avg_tvad_exp avg_printad_exp
	save tv_cpm_exp`yr', replace
}
