...
foreach d of local drg {
	...
	foreach yr of numlist 2000/2009 {	
		...
		*create frequency of each timeslot in each year
		bysort closest_timeslot_`d': egen `d'_freq = sum(i) if cpp_cpm=="DMA-CPP" & year==`yr' & avg_tvad_exp_`d'!=. 
		tab closest_timeslot_`d' if cpp_cpm=="DMA-CPP" & year==`yr' & avg_tvad_exp_`d'!=. 
		egen tag = tag(closest_timeslot_`d') if cpp_cpm=="DMA-CPP" & year==`yr' & avg_tvad_exp_`d'!=. 
		*count number of timeslots having cost closest to the average ad spending for each dma-year
		egen n_category = sum(tag) if cpp_cpm=="DMA-CPP" & year==`yr' & avg_tvad_exp_`d'!=. 
		codebook n_category 
		...
		while `k' > 5 {	
			...
			*store the frequency of each timeslot 
			bysort closest_timeslot_`d': egen `d'_freq = sum(i) if cpp_cpm=="DMA-CPP" & year==`yr' & avg_tvad_exp_`d'!=. 
			egen min_`d'_freq = min(`d'_freq) if cpp_cpm=="DMA-CPP" & year==`yr' & avg_tvad_exp_`d'!=. 
			...
			*for each DMA, find the time slot that has the second minimum difference
			...
			foreach t of local times {
				...
				gen diff_`t'_`d'2 = abs(avg_tvad_exp_`d' - cost_`t') if cpp_cpm=="DMA-CPP" & year==`yr' & avg_tvad_exp_`d'!=.  & diff_`t'_`d' > min_diff_`d' & `d'_freq == min_`d'_freq
			}
			#delimit ;
			...
			gen min_diff_`d'2 = min(diff_earlymorn_`d'2, diff_daytime_`d'2, diff_earlyfringe_`d'2, diff_earlynews_`d'2,
			 	 diff_primeaccess_`d'2, diff_primetime_`d'2, diff_latenews_`d'2, diff_latefringe_`d'2) if cpp_cpm=="DMA-CPP" & year==`yr' & avg_tvad_exp_`d'!=.  & `d'_freq == min_`d'_freq ;			
			#delimit cr
			...
			foreach diff of local difflist {
				replace closest_timeslot_`d'2 = "`diff'" if cpp_cpm=="DMA-CPP" & year==`yr' & avg_tvad_exp_`d'!=.  & `diff'_`d'2==min_diff_`d'2 & `d'_freq == min_`d'_freq 
			}
			...
			*replace closest timeslot and minimum difference for sparse cateogories with the second min value
			replace closest_timeslot_`d' = closest_timeslot_`d'2 if `d'_freq == min_`d'_freq & cpp_cpm=="DMA-CPP" & year==`yr' & avg_tvad_exp_`d'!=. 
			replace min_diff_`d' = min_diff_`d'2 if `d'_freq == min_`d'_freq & cpp_cpm=="DMA-CPP" & year==`yr' & avg_tvad_exp_`d'!=. 	 				
